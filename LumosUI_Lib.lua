...
Lumos Interface Suite (RayField Remaster)
by zod

Lumos Credits -
zod   | Designing + Programming
...
local Lumos = useStudio and script.Parent:FindFirstChild('Lumos') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned

repeat
	if Lumos:FindFirstChild('Build') and Lumos.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('Lumos | Build Mismatch')
		print('Lumos may encounter issues as you are running an incompatible interface version ('.. ((Lumos:FindFirstChild('Build') and Lumos.Build.Value) or 'No Build') ..').\n\nThis version of Lumos is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, Lumos = Lumos, useStudio and script.Parent:FindFirstChild('Lumos') or game:GetObjects("rbxassetid://10804731440")[1]
...
elseif syn and syn.protect_gui then 
	syn.protect_gui(Lumos)
	Lumos.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	Lumos.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Lumos.Name and Interface ~= Lumos then
			Interface.Enabled = false
			Interface.Name = "Lumos-Old"
		end
	end
elseif not useStudio then
...
if Lumos.AbsoluteSize.X < minSize.X and Lumos.AbsoluteSize.Y < minSize.Y then
...
local Main = Lumos.Main
local MPrompt = Lumos:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = Lumos:FindFirstChild('Drag')
...
local Notifications = Lumos.Notifications

local SelectedTheme = LumosLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = LumosLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	Lumos.Main.BackgroundColor3 = SelectedTheme.Background
	Lumos.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Lumos.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Lumos.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Lumos.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Lumos.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Lumos.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	Lumos.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	...
	for _, text in ipairs(Lumos:GetDescendants()) do
...
local function LoadConfiguration(Configuration)
	...
	for FlagName, Flag in pairs(LumosLibrary.Flags) do
		...
		if FlagValue then
			...
			warn("Lumos | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--LumosLibrary:Notify({Title = "Lumos Flags", Content = "Lumos was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(LumosLibrary.Flags) do
...
function LumosLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('Lumos | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			LumosLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show Lumos'.", Duration = 7, Image = 4400697855})
		else
			LumosLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping K.", Duration = 7, Image = 4400697855})
		end
	end
...
function LumosLibrary:CreateWindow(Settings)
	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				LumosLibrary:Notify({Title = 'Build Mismatch', Content = 'Lumos may encounter issues as you are running an incompatible interface version ('.. ((Lumos:FindFirstChild('Build') and Lumos.Build.Value) or 'No Build') ..').\n\nThis version of Lumos is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Lumos"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "Lumos Interface Suite" then
		LoadingFrame.Version.Text = "Lumos UI"
	end
	...
	if not Settings.DisableLumosPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				LumosLibrary:Notify({
					Title = "Lumos Interface",
	...
	if Settings.Discord and not useStudio then
		if isfolder and not isfolder(LumosFolder.."/Discord Invites") then
			makefolder(LumosFolder.."/Discord Invites")
		end

		if isfile and not isfile(LumosFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				pcall(function()
					request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(LumosFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Lumos RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(LumosFolder.."/Key System") then
			makefolder(LumosFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Lumos | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Lumos specific development.')
				end
			end
		end
		...
		if isfile and isfile(LumosFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(LumosFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(LumosFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						LumosLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Lumos | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Lumos specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= LumosLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					LumosLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Lumos | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Lumos specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					LumosLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("Lumos | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Lumos specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					LumosLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Lumos | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Lumos specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					LumosLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if ToggleSettings.CurrentValue == true then
				...
				if ToggleSettings.Color then
					Toggle.Switch.Indicator.UIStroke.Color = ToggleSettings.Color
					Toggle.Switch.Indicator.BackgroundColor3 = ToggleSettings.Color
					Toggle.Switch.UIStroke.Color = ToggleSettings.Color
				else
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				if ToggleSettings.CurrentValue == true then
					...
					if ToggleSettings.Color then
						TweenService:Create(Toggle.Switch.Indicator.UIStroke, TweenInfo.new(0.55, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Color = ToggleSettings.Color}):Play()
						TweenService:Create(Toggle.Switch.Indicator, TweenInfo.new(0.8, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {BackgroundColor3 = ToggleSettings.Color}):Play()
						TweenService:Create(Toggle.Switch.UIStroke, TweenInfo.new(0.55, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Color = ToggleSettings.Color}):Play()
					else
				...
				if not Success then
					...
					print("Lumos | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Lumos specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					LumosLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
			...
			Slider.Main.Progress:FindFirstChildOfClass("UICorner").CornerRadius = UDim.new(0,0)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Lumos | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with Lumos specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					LumosLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			LumosLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			LumosLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
...
function LumosLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function LumosLibrary:IsVisible(): boolean
	return not Hidden
end

function LumosLibrary:Destroy()
...
function LumosLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				LumosLibrary:Notify({Title = "Lumos Configurations", Content = "We couldn't enable Configuration Saving as you are not using file supported software.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			LumosLibrary:Notify({Title = "Lumos Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('Lumos Configurations Error | '..tostring(result))
			LumosLibrary:Notify({Title = "Lumos Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
end
...
if useStudio then
	...
	local Window = LumosLibrary:CreateWindow({
		Name = "Lumos Example Window",
		LoadingTitle = "Lumos Interface Suite",
		Theme = 'Default',
		Icon = 0,
		...
		KeySettings = {
			...
			FileName = "Key", -- It is recommended to use something unique as other scripts using Lumos may overwrite your key file
			SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
			GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like Lumos to get the key from
	...
	--LumosLibrary:Notify({Title = "Lumos Interface", Content = "Welcome to Lumos. These - are the brand new notification design for Lumos, with custom sizing and Lumos calculated wait times.", Image = 4483362458})
	...
	local thoptions = {}
	for themename, theme in pairs(LumosLibrary.Theme) do
...
task.delay(4, function() 
	LumosLibrary.LoadConfiguration()
